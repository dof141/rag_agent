...
def get_llm_client(model: Optional[str] = None, json_mode: bool = False) -> ChatOpenAI:
    ...
    extra_body = {}  # 千问专属：关闭思考链输出，减少冗余内容
    # 从配置中读取是否启用 extra_body 参数
    if hasattr(lm_config, "enable_thinking") and lm_config.enable_thinking is not None:
        extra_body["enable_thinking"] = lm_config.enable_thinking
...
def get_vlm_client(model: Optional[str] = None, timeout: Optional[float] = None) -> ChatOpenAI:
    """
    获取独立的视觉多模态大模型 (VLM) ChatOpenAI 客户端实例
    特性：
    - 独立读取 VLM_BASE_URL 与 VLM_API_KEY，不与 LLM 主对话接口共用链路
    - 独立配置 request_timeout 请求超时时间，防止视觉解析超时阻塞全局链条
    - 使用独立的 _vlm_client_cache 缓存池
    """
    target_model = model or lm_config.lv_model or "qwen-vl-plus"
    target_timeout = timeout or lm_config.vlm_timeout or 60.0
    cache_key = (target_model, target_timeout)

    if cache_key in _vlm_client_cache:
        logger.debug(f"[VLM客户端] 缓存命中，直接返回视觉实例：模型={target_model}，超时={target_timeout}s")
        return _vlm_client_cache[cache_key]

    vlm_base_url = lm_config.vlm_base_url or lm_config.base_url
    vlm_api_key = lm_config.vlm_api_key or lm_config.api_key

    if not vlm_api_key:
        raise ValueError("[VLM客户端] 配置缺失：请在.env中配置 VLM_API_KEY 或 OPENAI_API_KEY")
    if not vlm_base_url:
        raise ValueError("[VLM客户端] 配置缺失：请在.env中配置 VLM_BASE_URL 或 OPENAI_BASE_URL")

    logger.info(f"[VLM客户端] 开始初始化独立视觉大模型实例：模型={target_model}，URL={vlm_base_url}，超时={target_timeout}s")
    ...
    try:
        vlm_client = ChatOpenAI(
            ...
            request_timeout=target_timeout,
        )
    except LangChainException as e:
        raise Exception(f"[VLM客户端] 视觉模型【{target_model}】初始化失败：{str(e)}") from e

    _vlm_client_cache[cache_key] = vlm_client
    logger.info(f"[VLM客户端] 实例初始化成功并缓存：模型={target_model}")
    return vlm_client
